...
if [ "$1" = "" ]
then
  echo "Usage: gen1.sh <termOption> <labelOption> <inputFile> <outputFile>"
  exit
fi
...
######### gera os plot/replots
echo "./gen2.sh $inputfile $attribNumber >> __tmp3.scr"  > ./__temp_exec.sh
...
echo "sed 's/<title>/Data from $inputfile/' < __tmp2.scr > __tmp1.scr" >> __temp_exec.sh
