...
public class FormulaReferenceClassicalProofTree extends ClassicalProofTree {
    ...
    // TIREI EM 26/01/2005 - não vi necessidade
...
//        _subformulaLocalReferencesInCandidates = new FormulaSubformulaReferencesMultiMap();
    ...
    private void updateReferences(SignedFormulaNode aNode) {
        updateLocalReferences((SignedFormula) aNode.getContent(),
                _localReferences, _subformulaLocalReferences);
        // TIREI EM 26/01/2005 - não vi necessidade
        // updateLocalReferences((SignedFormula) aNode.getContent(),
    ...
    public void tryToCloseParent() {
        ...
        if ((_parent != null) && (this == _parent.getRight())
            ...
            while (_current != null && !_current._closed) {
                ...
                if (_previous == (FormulaReferenceClassicalProofTree) _current
                        .getRight()) {
                    ((FormulaReferenceClassicalProofTree) _current)._closed = true;
                    _previous = _current;
                    _current = (FormulaReferenceClassicalProofTree) _current
                            .getParent();
                } else {
...
//    	System.err.println("ADDED NODE "+aNode);
    ...
    protected void setOtherStructures(IProofTree pt, INode aNode) {
        ...
        ((FormulaReferenceClassicalProofTree) pt)
                .updateReferences((SignedFormulaNode) aNode);
    }
    ...
    private void updateLocalReferences(SignedFormula signedFormula,
        ...
        putAllSubformulasInLocalReferences(signedFormula.getFormula(),
                signedFormula, references, subformulaReferences);
    }
    ...
    private void putAllSubformulasInLocalReferences(Formula formula,
            SignedFormula sf, FormulaSignedFormulaMultiMap references,
            FormulaSubformulaReferencesMultiMap subformulaReferences) {
        if (formula.getImmediateSubformulas().size() != 0) {
            for (int i = 0; i < formula.getImmediateSubformulas().size(); i++) {
                subformulaReferences.put((Formula) formula
                        .getImmediateSubformulas().get(i), sf, formula);
                references.put((Formula) formula.getImmediateSubformulas().get(
                        i), sf);
                putAllSubformulasInLocalReferences((Formula) formula
                        .getImmediateSubformulas().get(i), sf, references,
    ...
    public SignedFormulaList getParentReferences(Formula formula) {
        ...
        do {
            sfl.addAll(((FormulaReferenceClassicalProofTree) current)
     ...
     *         branch or up.
     */
    public FormulaList getSubformulaParentReferences(Formula formula,
            SignedFormula sf) {
        ...
        do {
            fl.addAll(((FormulaReferenceClassicalProofTree) current)
                    .getSubformulaLocalReferences(formula, sf));
            current = current.getParent();
        } while (current != null);
    ...
    public void addBackjumpingFormula(SignedFormula sf) {
        SignedFormulaNode aNode = new SignedFormulaNode(sf,
                SignedFormulaNodeState.NOT_ANALYSED, Origin.EMPTY);
...
//        getSignedFormulasToNodes().put(sf, aNode);
    ...
    public void removeFromAllStructures(SignedFormula sformula) {
        ...
        if (getNode(sformula).getPrevious() != null) {
            getNode(sformula).getPrevious()
                    .setNext(getNode(sformula).getNext());
        } else {
    ...
    public void removeReferencesInStructures(SignedFormulaNode node) {
        ...
        removeAllSubformulasInLocalReferences(sf.getFormula(), sf, _localReferences, _subformulaLocalReferences);
    }
    
    private void removeAllSubformulasInLocalReferences(Formula formula,
            SignedFormula sf, FormulaSignedFormulaMultiMap references,
            FormulaSubformulaReferencesMultiMap subformulaReferences) {
        if (formula.getImmediateSubformulas().size() != 0) {
            for (int i = 0; i < formula.getImmediateSubformulas().size(); i++) {
                subformulaReferences.remove((Formula) formula
                        .getImmediateSubformulas().get(i), sf, formula);
                references.remove((Formula) formula.getImmediateSubformulas().get(
                        i), sf);
                removeAllSubformulasInLocalReferences((Formula) formula
                        .getImmediateSubformulas().get(i), sf, references,
                        subformulaReferences);
            }
        }
...
//			System.err.println("REMOVING "+(SignedFormula)((SignedFormulaNode)current).getContent()); 
			((FormulaReferenceClassicalProofTree) getLeft()).removeSignedFormula((SignedFormula)((SignedFormulaNode)current).getContent());
		}

		// removes root
		
		((FormulaReferenceClassicalProofTree) getLeft()).removeSignedFormula((SignedFormula)((SignedFormulaNode)getLeft().getRoot()).getContent());

//		System.err.println(getLeft().getRoot());
//		System.err.println(getRight().getRoot());
		
//		recoverInPbCandidatesHereAndAbove((SignedFormulaNode) getLeft().getRoot()) 
	...
	public void removeRight(){
		...
		while(current.getNext()!=null){
			current = current.getNext();
//			System.err.println("REMOVING "+(SignedFormula)((SignedFormulaNode)current).getContent()); 
			((FormulaReferenceClassicalProofTree) getRight()).removeSignedFormula((SignedFormula)((SignedFormulaNode)current).getContent());
		}

		// removes root
		((FormulaReferenceClassicalProofTree) getRight()).removeSignedFormula((SignedFormula)((SignedFormulaNode)getRight().getRoot()).getContent());
